...
export const exportTimetableToPDFTable = (
  data,
  programName,
  ...
  try {
    ...
    if (showAllPrograms) {
      const programs = [...new Set(data.map((item) => item.program_code))]
        .filter(Boolean)
        .sort();
      ...
      programs.forEach((program, index) => {
        ...
        const programData = data.filter(
          (item) => item.program_code === program
        );
        generateProgramTableManual(pdf, programData, program);
      });
    } else {
      const programData = data.filter(
        (item) => item.program_code === programName
      );
...
function generateProgramTableManual(pdf, programData, programName) {
  ...
  programData.forEach((item) => {
    if (
      timetableGrid[item.Time] &&
      timetableGrid[item.Time].hasOwnProperty(item.Day)
    ) {
  ...
  pdf.text("Time", startX + timeColWidth / 2, startY + headerHeight / 2 + 2, {
    align: "center",
  });
  ...
  days.forEach((day, index) => {
    ...
    pdf.text(day, x + dayColWidth / 2, startY + headerHeight / 2 + 2, {
      align: "center",
    });
  });
  ...
  timeSlots.forEach((timeSlot, rowIndex) => {
    ...
    days.forEach((day, colIndex) => {
      ...
      if (cellData) {
        ...
        pdf.setFontSize(8);
        const courseName =
          cellData.course.length > 25
            ? cellData.course.substring(0, 22) + "..."
            : cellData.course;
        ...
        pdf.setFontSize(7);
        const facultyName =
          cellData.faculty.length > 20
            ? cellData.faculty.substring(0, 17) + "..."
            : cellData.faculty;
        ...
        pdf.text("Free", x + dayColWidth / 2, y + cellHeight / 2 + 1, {
          align: "center",
        });
        pdf.setTextColor(0, 0, 0);
      }
  ...
  const legendStartY =
    startY + headerHeight + timeSlots.length * cellHeight + 15;
...
export const exportTimetableToExcelFormatted = (
  data,
  programName,
  showAllPrograms = false
) => {
...
function createSummarySheet(workbook, data, programs) {
  const summaryData = [
    ...
    ...programs.map((program) => {
      ...
      const uniqueCourses = [
        ...new Set(programData.map((item) => item.course_name)),
      ].length;
      const uniqueFaculty = [
        ...new Set(programData.map((item) => item.faculty_name)),
      ].length;
      const uniqueRooms = [
        ...new Set(programData.map((item) => item.room_name)),
      ].length;
      return [
        program,
        programData.length,
